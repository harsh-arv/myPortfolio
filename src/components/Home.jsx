...
const NetflixHero = ({ brand }) => (
  <div name="home" className="min-h-screen w-full relative overflow-hidden"
    ...
    <div className="relative w-full px-8 md:px-16 lg:px-24 xl:px-32 flex flex-col justify-center h-screen max-w-3xl">
      <span className="inline-flex items-center gap-2 bg-t-primary/20 text-t-primary px-3 py-1 rounded text-sm font-bold tracking-widest mb-4 w-fit">
        N &nbsp; {brand.heroGreeting}
      </span>
      <h1 className="text-5xl sm:text-7xl font-black text-t-text mb-2 leading-tight">
        Harsh Gandharv
      </h1>
      <div className="text-2xl sm:text-3xl font-bold text-t-primary mb-4 h-10">
        <TypeAnimation
      ...
      <div className="flex gap-4 mb-8">
        <Link to="portfolio" smooth duration={500}
          className="flex items-center gap-2 bg-white text-black px-8 py-3 rounded font-bold text-lg hover:bg-gray-200 transition cursor-pointer">
          <FaPlay /> Play
        </Link>
        <Link to="about" smooth duration={500}
          className="flex items-center gap-2 bg-gray-500/50 text-white px-8 py-3 rounded font-bold text-lg hover:bg-gray-500/70 transition cursor-pointer">
          <FaPlus /> My List
        </Link>
      </div>

      {/* Netflix-style tags */}
      <div className="flex items-center gap-4 text-sm text-t-text-secondary">
...
const FacebookHero = ({ brand }) => (
  <div name="home" className="min-h-screen w-full"
    ...
    <div className="w-full pt-20">
      ...
      {/* Profile section */}
      <div className="w-full px-8 md:px-16 lg:px-24 xl:px-32 -mt-20 relative">
        <div className="flex flex-col md:flex-row items-end md:items-end gap-6">
          <div className="relative">
            <div className="w-40 h-40 rounded-full border-4 border-t-card overflow-hidden bg-t-card shadow-xl">
              <img src={HeroImage} alt="Harsh Gandharv" className="w-full h-full object-cover" />
            </div>
            <div className="absolute bottom-2 right-2 w-8 h-8 bg-green-500 rounded-full border-4 border-t-card" />
          </div>
          <div className="flex-1 pb-4">
            <h1 className="text-4xl sm:text-5xl font-bold text-t-text">Harsh Gandharv</h1>
        ...
        <div className="flex gap-3 mt-6 border-t border-t-border pt-4">
          <Link to="portfolio" smooth duration={500}
            className="bg-t-primary text-white px-6 py-2 rounded-lg font-semibold hover:opacity-90 transition cursor-pointer">
            View Projects
          </Link>
          <Link to="contact" smooth duration={500}
            className="bg-t-card text-t-text px-6 py-2 rounded-lg font-semibold border border-t-border hover:bg-t-alt transition cursor-pointer">
            Send Message
          </Link>
          <a href="/resume.pdf" download
            className="bg-t-card text-t-text px-6 py-2 rounded-lg font-semibold border border-t-border hover:bg-t-alt transition flex items-center gap-2">
            <FaDownload size={14} /> Resume
          </a>
        </div>

        {/* Stats as fb-style counters */}
        <div className="flex gap-8 mt-6 pb-8 border-b border-t-border">
...
const AmazonHero = ({ brand }) => (
  <div name="home" className="min-h-screen w-full"
    style={{ background: "linear-gradient(to bottom, var(--c-bg-base), var(--c-bg-alt))" }}>
    <div className="w-full px-8 md:px-16 lg:px-24 xl:px-32 flex flex-col md:flex-row items-center justify-center h-screen gap-12">
      {/* Product info */}
      <div className="flex-1 max-w-2xl">
        ...
        <div className="flex gap-3">
          <Link to="contact" smooth duration={500}
            className="bg-t-primary text-white px-8 py-3 rounded-full font-bold hover:opacity-90 transition cursor-pointer flex items-center gap-2 text-sm">
            <FaShoppingCart /> Hire Now
          </Link>
          <Link to="portfolio" smooth duration={500}
            className="bg-t-card text-t-text px-8 py-3 rounded-full font-bold border border-t-border hover:bg-t-alt transition cursor-pointer text-sm">
            View Projects
          </Link>
        </div>
      </div>
      ...
      <div className="flex-shrink-0 flex items-center justify-center">
        <div className="bg-t-card border border-t-border rounded-lg p-6 shadow-xl">
          <img src={HeroImage} alt="Harsh Gandharv" className="w-64 md:w-80 rounded-lg" />
        </div>
      </div>
    </div>
  </div>
...
const SpotifyHero = ({ brand }) => (
  <div name="home" className="min-h-screen w-full"
    ...
    <div className="w-full px-8 md:px-16 lg:px-24 xl:px-32 flex flex-col md:flex-row items-end justify-start pt-32 pb-12 gap-8">
      {/* Album art */}
      <div className="flex-shrink-0">
        <img src={HeroImage} alt="Harsh Gandharv"
          className="w-56 h-56 md:w-64 md:h-64 rounded shadow-2xl object-cover" />
      </div>
    ...
    <div className="w-full px-8 md:px-16 lg:px-24 xl:px-32 flex items-center gap-6 py-6">
      <Link to="portfolio" smooth duration={500}
...
const DefaultHero = ({ brand }) => (
  <div name="home" className="min-h-screen w-full"
    style={{ background: "linear-gradient(to bottom, var(--c-bg-base), var(--c-bg-base), var(--c-bg-alt))" }}>
    <div className="w-full px-8 md:px-16 lg:px-24 xl:px-32 flex flex-col items-center justify-center h-screen md:flex-row">
      <div className="flex flex-col justify-center h-full">
        <p className="text-t-accent text-xl mb-2">{brand.heroGreeting}</p>
        <h1 className="text-4xl sm:text-6xl font-bold text-t-text mb-4">Harsh Gandharv</h1>
        <div className="text-3xl sm:text-5xl font-bold text-t-primary mb-6 h-16">
          <TypeAnimation
            sequence={['Full Stack Developer', 2000, 'Java Expert', 2000, 'React Developer', 2000, 'Microservices Architect', 2000]}
            wrapper="span" speed={50} repeat={Infinity}
          />
        </div>
        <p className="text-t-text-secondary py-4 max-w-md leading-relaxed">
        ...
        <div className="flex gap-4 mt-4">
          <Link to="portfolio" smooth duration={500}
            className="group text-white w-fit px-6 py-3 flex items-center rounded-md bg-t-primary cursor-pointer hover:scale-105 transition-transform duration-300">
            View Projects
            <span className="group-hover:rotate-90 duration-300"><MdOutlineKeyboardArrowRight size={25} className="ml-1" /></span>
          </Link>
          <a href="/resume.pdf" download
            className="group text-t-text w-fit px-6 py-3 flex items-center rounded-md border-2 border-t-primary hover:bg-t-primary hover:text-white cursor-pointer transition-all duration-300">
            <FaDownload size={20} className="mr-2" /> Resume
          </a>
        </div>
        <div className="flex gap-8 mt-8">
          <div><p className="text-3xl font-bold text-t-accent">4+</p><p className="text-t-text-muted text-sm">Years Exp</p></div>
          <div><p className="text-3xl font-bold text-t-accent">4</p><p className="text-t-text-muted text-sm">Companies</p></div>
          <div><p className="text-3xl font-bold text-t-accent">15+</p><p className="text-t-text-muted text-sm">Technologies</p></div>
        </div>
      </div>
      <div className="flex items-center justify-center">
        <div className="relative">
          <div className="absolute inset-0 bg-t-primary rounded-2xl blur-xl opacity-30 animate-pulse"></div>
          <img src={HeroImage} alt="Harsh Gandharv"
            className="relative rounded-2xl mx-auto w-2/3 md:w-full border-4 border-t-border-accent shadow-2xl" />
        </div>
      </div>
    </div>
  </div>
